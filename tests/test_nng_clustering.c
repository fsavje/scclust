...
void scc_ut_nng_free_SeedClustering(void** state)
{
	(void) state;
	...
	assert_int_equal(seed_clustering.num_data_points, 0);
	...
	iscc_SeedClustering seed_clustering2 = iscc_get_seed_clustering(&nng, SCC_FINDSEEDS_LEXICAL, 6, ext_cluster_label);
	...
	assert_int_equal(seed_clustering2.num_data_points, 0);
...
void scc_ut_nng_copy_SeedClustering(void** state)
{
	(void) state;
	...
	iscc_SeedClustering copy = iscc_copy_seed_clustering(&seed_clustering);

	assert_int_equal(seed_clustering.num_data_points, copy.num_data_points);
	...
	assert_ptr_not_equal(seed_clustering.assigned, copy.assigned);
	assert_ptr_not_equal(seed_clustering.seeds, copy.seeds);
	assert_ptr_not_equal(seed_clustering.cluster_label, copy.cluster_label);
	assert_memory_equal(seed_clustering.assigned, copy.assigned, seed_clustering.num_data_points * sizeof(bool));
	assert_memory_equal(seed_clustering.seeds, copy.seeds, seed_clustering.seed_capacity * sizeof(scc_Dpid));
	assert_memory_equal(seed_clustering.cluster_label, copy.cluster_label, seed_clustering.num_data_points * sizeof(scc_Clabel));

	seed_clustering.external_labels = true;
	iscc_SeedClustering copy2 = iscc_copy_seed_clustering(&seed_clustering);
	seed_clustering.external_labels = false;

	assert_int_equal(seed_clustering.num_data_points, copy2.num_data_points);
	...
	assert_false(copy2.external_labels);
	assert_non_null(copy2.assigned);
	assert_non_null(copy2.seeds);
	assert_non_null(copy2.cluster_label);
	assert_ptr_not_equal(seed_clustering.assigned, copy2.assigned);
	assert_ptr_not_equal(seed_clustering.seeds, copy2.seeds);
	assert_ptr_not_equal(seed_clustering.cluster_label, copy2.cluster_label);
	assert_memory_equal(seed_clustering.assigned, copy2.assigned, seed_clustering.num_data_points * sizeof(bool));
	assert_memory_equal(seed_clustering.seeds, copy2.seeds, seed_clustering.seed_capacity * sizeof(scc_Dpid));
	assert_memory_equal(seed_clustering.cluster_label, copy2.cluster_label, seed_clustering.num_data_points * sizeof(scc_Clabel));

	iscc_free_digraph(&nng);
	iscc_free_seed_clustering(&copy);
	iscc_free_seed_clustering(&copy2);
	iscc_free_seed_clustering(&seed_clustering);
}
...
void scc_ut_ignore_remaining(void** state)
{
	...
	bool assigned[20] = { true,  false, true,  false, true, 
	                      ...
	                      false, true,  false, true,  false  };
	scc_Dpid seeds[5] = { 0, 18, 2, 6, 4 };
	scc_Clabel cluster_label[20] = { 0, 0, 2, 1, 4,
	                                 2, 3, 3, 0, 4,
	                                 1, 5, 4, 6, 3,
	                                 7, 2, 8, 1, 9  };
	iscc_SeedClustering cl_template = {
		.num_data_points = 20,
		.num_clusters = 5,
		.seed_capacity = 5,
	...
	iscc_SeedClustering ref = iscc_copy_seed_clustering(&cl_template);
	iscc_SeedClustering cl = cl_template;
	scc_Clustering out = iscc_ignore_remaining(&cl);
	assert_int_equal(cl.num_data_points, 20);
	assert_int_equal(cl.num_clusters, 5);
	assert_int_equal(cl.seed_capacity, 5);
	assert_true(cl.external_labels);
	assert_non_null(cl.assigned);
	assert_memory_equal(cl.assigned, ref.assigned, cl.num_data_points * sizeof(bool));
	assert_non_null(cl.seeds);
	assert_memory_equal(cl.seeds, ref.seeds, cl.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl.cluster_label);
	assert_ptr_equal(cl.cluster_label, out.cluster_label);
	assert_memory_equal(cl.cluster_label, ref_cluster_label, cl.num_data_points * sizeof(scc_Clabel));

	assert_int_equal(out.num_data_points, 20);
	assert_int_equal(out.num_clusters, 5);
	assert_true(out.external_labels);
	assert_non_null(out.cluster_label);
	assert_ptr_equal(out.cluster_label, cluster_label);
	assert_memory_equal(out.cluster_label, ref_cluster_label, 20 * sizeof(scc_Clabel));

	iscc_free_seed_clustering(&ref);
...
void scc_ut_assign_remaining_lexical(void** state)
{
	...
	scc_Dpid head1[20] = { 4,
	                     ...
	                     12,
	                      0 };
	iscc_Digraph prio1 = iscc_digraph_from_pieces(20, 20, tail_ptr1, head1);
	assert_true(iscc_digraph_is_initialized(&prio1));
	...
	scc_Dpid head2[34] = {  0, 19,
	                      ...
	                      13, 15, 17,
	                       ...
	                       9, 15,
	                       0,  2, 4, 6, 8, 10 };
	iscc_Digraph prio2 = iscc_digraph_from_pieces(20, 34, tail_ptr2, head2);
	assert_true(iscc_digraph_is_initialized(&prio2));
	...
	iscc_SeedClustering cl1 = iscc_copy_seed_clustering(&cl_template);
	scc_Clustering out1 = iscc_assign_remaining_lexical(&cl1, &prio1);
	assert_int_equal(cl1.num_data_points, 20);
	assert_int_equal(cl1.num_clusters, 5);
	assert_int_equal(cl1.seed_capacity, 5);
	assert_true(cl1.external_labels);
	assert_non_null(cl1.assigned);
	assert_memory_equal(cl1.assigned, assigned, cl1.num_data_points * sizeof(bool));
	assert_non_null(cl1.seeds);
	assert_memory_equal(cl1.seeds, seeds, cl1.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl1.cluster_label);
	assert_ptr_equal(cl1.cluster_label, out1.cluster_label);
	assert_memory_equal(cl1.cluster_label, ref_cluster_label1, cl1.num_data_points * sizeof(scc_Clabel));

	assert_int_equal(out1.num_data_points, 20);
	assert_int_equal(out1.num_clusters, 5);
	assert_false(out1.external_labels);
	assert_non_null(out1.cluster_label);
	assert_ptr_not_equal(out1.cluster_label, cl_template.cluster_label);
	assert_memory_equal(out1.cluster_label, ref_cluster_label1, 20 * sizeof(scc_Clabel));

	iscc_SeedClustering cl2 = iscc_copy_seed_clustering(&cl_template);
	scc_Clustering out2 = iscc_assign_remaining_lexical(&cl2, &prio2);
	assert_int_equal(cl2.num_data_points, 20);
	assert_int_equal(cl2.num_clusters, 5);
	assert_int_equal(cl2.seed_capacity, 5);
	assert_true(cl2.external_labels);
	assert_non_null(cl2.assigned);
	assert_memory_equal(cl2.assigned, assigned, cl2.num_data_points * sizeof(bool));
	assert_non_null(cl2.seeds);
	assert_memory_equal(cl2.seeds, seeds, cl2.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl2.cluster_label);
	assert_ptr_equal(cl2.cluster_label, out2.cluster_label);
	assert_memory_equal(cl2.cluster_label, ref_cluster_label2, cl2.num_data_points * sizeof(scc_Clabel));

	assert_int_equal(out2.num_data_points, 20);
	assert_int_equal(out2.num_clusters, 5);
	assert_false(out2.external_labels);
	assert_non_null(out2.cluster_label);
	assert_ptr_not_equal(out2.cluster_label, cl_template.cluster_label);
	assert_memory_equal(out2.cluster_label, ref_cluster_label2, 20 * sizeof(scc_Clabel));

	iscc_free_digraph(&prio1);
	iscc_free_seed_clustering(&cl1);
	scc_free_clustering(&out1);
	iscc_free_digraph(&prio2);
...
void scc_ut_assign_remaining_desired_size(void** state)
{
	...
	scc_Clabel cluster_label1[20] = { 0, 0, 2, 1, 4,
	                                  2, 3, 3, 0, 4,
	                                  1, 5, 4, 6, 3,
	                                  7, 2, 8, 1, 9  };
	scc_Clabel cluster_label2[20] = { 0, 0, 2, 1, 4,
	                                  2, 3, 3, 0, 4,
	                                  1, 5, 4, 6, 3,
	                                  7, 2, 8, 1, 9  };
	scc_Clabel cluster_label3[20] = { 0, 0, 2, 1, 4,
	                                  2, 3, 3, 0, 4,
	                                  1, 5, 4, 6, 3,
	                                  7, 2, 8, 1, 9  };
	scc_Clabel cluster_label4[20] = { 0, 0, 2, 1, 4,
	                                  2, 3, 3, 0, 4,
	                                  1, 5, 4, 6, 3,
	                                  7, 2, 8, 1, 9  };
	...
	scc_Clabel ref_cluster_label3[20] = { 0, 1, 2, M, 4,
	                                      ...
	                                      1, 0, 4, M, 3,
	                                      M, 2, 3, 1, 0  };
	scc_Clabel ref_cluster_label4[20] = { 0, 1, 2, M, 4,
	                                      1, 3, 0, 0, M,
	                                      1, 0, 4, M, 3,
	                                      M, 2, 0, 1, 0  };
	...
	iscc_SeedClustering cl1 = iscc_copy_seed_clustering(&cl_template);
	cl1.external_labels = true;
	cl1.cluster_label = cluster_label1;
	scc_Clustering out1 = iscc_assign_remaining_desired_size(&cl1, &prio1, 2);
	assert_int_equal(cl1.num_data_points, 20);
	assert_int_equal(cl1.num_clusters, 5);
	assert_int_equal(cl1.seed_capacity, 5);
	assert_true(cl1.external_labels);
	...
	assert_memory_equal(cl1.seeds, seeds, cl1.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl1.cluster_label);
	assert_ptr_equal(cl1.cluster_label, cluster_label1);
	assert_memory_equal(cl1.cluster_label, ref_cluster_label1, cl1.num_data_points * sizeof(scc_Clabel));
	...
	assert_non_null(out1.cluster_label);
	assert_ptr_equal(out1.cluster_label, cluster_label1);
	assert_memory_equal(out1.cluster_label, ref_cluster_label1, 20 * sizeof(scc_Clabel));

	iscc_SeedClustering cl2 = iscc_copy_seed_clustering(&cl_template);
	cl2.external_labels = true;
	cl2.cluster_label = cluster_label2;
	scc_Clustering out2 = iscc_assign_remaining_desired_size(&cl2, &prio2, 2);
	assert_int_equal(cl2.num_data_points, 20);
	assert_int_equal(cl2.num_clusters, 5);
	assert_int_equal(cl2.seed_capacity, 5);
	assert_true(cl2.external_labels);
	...
	assert_memory_equal(cl2.seeds, seeds, cl2.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl2.cluster_label);
	assert_ptr_equal(cl2.cluster_label, cluster_label2);
	assert_memory_equal(cl2.cluster_label, ref_cluster_label2, cl2.num_data_points * sizeof(scc_Clabel));
	...
	assert_non_null(out2.cluster_label);
	assert_ptr_equal(out2.cluster_label, cluster_label2);
	assert_memory_equal(out2.cluster_label, ref_cluster_label2, 20 * sizeof(scc_Clabel));

	iscc_SeedClustering cl3 = iscc_copy_seed_clustering(&cl_template);
	cl3.external_labels = true;
	cl3.cluster_label = cluster_label3;
	scc_Clustering out3 = iscc_assign_remaining_desired_size(&cl3, &prio2, 3);
	assert_int_equal(cl3.num_data_points, 20);
	assert_int_equal(cl3.num_clusters, 5);
	assert_int_equal(cl3.seed_capacity, 5);
	assert_true(cl3.external_labels);
	assert_non_null(cl3.assigned);
	assert_memory_equal(cl3.assigned, assigned, cl3.num_data_points * sizeof(bool));
	assert_non_null(cl3.seeds);
	assert_memory_equal(cl3.seeds, seeds, cl3.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl3.cluster_label);
	assert_ptr_equal(cl3.cluster_label, cluster_label3);
	assert_memory_equal(cl3.cluster_label, ref_cluster_label3, cl3.num_data_points * sizeof(scc_Clabel));

	assert_int_equal(out3.num_data_points, 20);
	assert_int_equal(out3.num_clusters, 5);
	assert_true(out3.external_labels);
	assert_non_null(out3.cluster_label);
	assert_ptr_equal(out3.cluster_label, cluster_label3);
	assert_memory_equal(out3.cluster_label, ref_cluster_label3, 20 * sizeof(scc_Clabel));

	iscc_SeedClustering cl4 = iscc_copy_seed_clustering(&cl_template);
	cl4.external_labels = true;
	cl4.cluster_label = cluster_label4;
	scc_Clustering out4 = iscc_assign_remaining_desired_size(&cl4, &prio2, 0);
	assert_int_equal(cl4.num_data_points, 20);
	assert_int_equal(cl4.num_clusters, 5);
	assert_int_equal(cl4.seed_capacity, 5);
	assert_true(cl4.external_labels);
	assert_non_null(cl4.assigned);
	assert_memory_equal(cl4.assigned, assigned, cl4.num_data_points * sizeof(bool));
	assert_non_null(cl4.seeds);
	assert_memory_equal(cl4.seeds, seeds, cl4.seed_capacity * sizeof(scc_Dpid));
	assert_non_null(cl4.cluster_label);
	assert_ptr_equal(cl4.cluster_label, cluster_label4);
	assert_memory_equal(cl4.cluster_label, ref_cluster_label4, cl4.num_data_points * sizeof(scc_Clabel));

	assert_int_equal(out4.num_data_points, 20);
	assert_int_equal(out4.num_clusters, 5);
	assert_true(out4.external_labels);
	assert_non_null(out4.cluster_label);
	assert_ptr_equal(out4.cluster_label, cluster_label4);
	assert_memory_equal(out4.cluster_label, ref_cluster_label4, 20 * sizeof(scc_Clabel));
	...
	iscc_free_seed_clustering(&cl2);
	scc_free_clustering(&out2);
	iscc_free_seed_clustering(&cl3);
	scc_free_clustering(&out3);
	iscc_free_seed_clustering(&cl4);
...
int main(void)
{
	const struct CMUnitTest test_nng_clustering[] = {
		cmocka_unit_test(scc_ut_nng_free_SeedClustering),
		cmocka_unit_test(scc_ut_nng_copy_SeedClustering),
